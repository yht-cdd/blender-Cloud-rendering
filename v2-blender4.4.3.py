...
def fix_missing_alembic_files():
    """修复缺失的 Alembic 文件路径，支持用户指定新目录"""
    print("🔄 检查 Alembic 文件有效性...")
    ...
    for obj in bpy.data.objects:
        for mod in obj.modifiers:
            if mod.type == 'MESH_SEQUENCE_CACHE':
                ...
                if cache_file and cache_file.filepath:
                    filepath = bpy.path.abspath(cache_file.filepath)
                    if not os.path.exists(filepath):
                        missing_files.add(cache_file.filepath)
                else:
                    missing_files.add("<未知路径>")

    if not missing_files:
        print("✅ 所有 Alembic 文件均存在")
        return

    print("⚠️ 发现以下缺失的 Alembic 文件：")
    for mf in missing_files:
        print(f"  - {mf}")

    # 让用户输入新目录
    new_dir = input("请输入 Alembic 文件所在的新目录路径（留空则跳过修复）: ").strip()
    if not new_dir:
        print("🚪 跳过修复，禁用所有缺失缓存的修改器")
        disable_all_missing_alembic_modifiers()
        return
    ...
    for obj in bpy.data.objects:
        for mod in obj.modifiers:
            if mod.type == 'MESH_SEQUENCE_CACHE':
                cache_file = mod.cache_file
                if cache_file and cache_file.filepath:
                    old_path = bpy.path.abspath(cache_file.filepath)
                    if not os.path.exists(old_path):
                        filename = os.path.basename(cache_file.filepath)
                        new_path = os.path.join(new_dir, filename)
                        if os.path.exists(new_path):
                            print(f"✅ 修复: {cache_file.filepath} → {new_path}")
                            cache_file.filepath = new_path
                            fixed_count += 1
                        else:
                            print(f"⚠️ 仍未找到: {new_path}，禁用修改器")
                            mod.show_render = False
                            mod.show_viewport = False

    print(f"✅ 已修复 {fixed_count} 个 Alembic 文件路径")

def disable_all_missing_alembic_modifiers():
    """禁用所有缺失 Alembic 文件的修改器（ fallback ）"""
    disabled_count = 0
    for obj in bpy.data.objects:
        for mod in obj.modifiers:
            if mod.type == 'MESH_SEQUENCE_CACHE':
                ...
                if cache_file and cache_file.filepath:
                    ...
                    mod.show_viewport = False
                    disabled_count += 1
    print(f"🚪 已禁用 {disabled_count} 个缺失 Alembic 的修改器")
# =====================================================================
# 预设配置
# =====================================================================
DEFAULT_CONFIG = {
...
def get_blend_config():
    ...
    if scene.render.engine == 'CYCLES':
        ...
        # 修复：noise_threshold 属性在 4.4.3 中可能已移除
        if hasattr(scene.cycles, 'noise_threshold'):
            config["noise_threshold"] = scene.cycles.noise_threshold
        else:
            config["noise_threshold"] = DEFAULT_CONFIG["noise_threshold"]
        
        # 修复：denoising_samples 属性在 4.4.3 中可能已移除
        if hasattr(scene.cycles, 'denoising_samples'):
            config["denoising_samples"] = scene.cycles.denoising_samples
        else:
            config["denoising_samples"] = DEFAULT_CONFIG["denoising_samples"]
        ...
        # 修复：Bloom 效果在 4.4.3 中已移除
        config["eevee_bloom"] = False  # 默认值
        
        # 修复：SSR 效果在 4.4.3 中已移除
        config["eevee_ssr"] = False  # 默认值
        
        # 修复：体积效果在 4.4.3 中已移除
        config["eevee_volumetric"] = False  # 默认值
        
        # 检查是否启用了光线追踪反射
        if hasattr(eevee_settings, 'raytrace_reflections'):
            config["eevee_ssr"] = eevee_settings.raytrace_reflections.enabled
        
        # 检查是否启用了体积效果
        if hasattr(eevee_settings, 'use_volumetrics'):
            config["eevee_volumetric"] = eevee_settings.use_volumetrics
        elif hasattr(eevee_settings, 'volumetric'):
            config["eevee_volumetric"] = eevee_settings.volumetric.enabled
    ...
    if config["file_format"] == "FFMPEG":
        ...
        crf_value = scene.render.ffmpeg.constant_rate_factor
        if crf_value <= 0:
            config["ffmpeg_quality"] = "LOSSLESS"
        elif crf_value <= 18:
            config["ffmpeg_quality"] = "PERC_LOSSLESS"
        elif crf_value <= 20:
            config["ffmpeg_quality"] = "HIGH"
        elif crf_value <= 23:
            config["ffmpeg_quality"] = "MEDIUM"
        else:
            config["ffmpeg_quality"] = "LOW"
...
def apply_config(config):
    ...
    if config["render_engine"] == "CYCLES":
        ...
        # 修复：Bloom 效果在 4.4.3 中已移除
        # 只有在用户明确要求时才设置 Bloom 效果
        if config["eevee_bloom"]:
            # 确保使用合成器
        ...
        # 设置光线追踪反射
        if hasattr(eevee_settings, 'raytrace_reflections'):
            eevee_settings.raytrace_reflections.enabled = config["eevee_ssr"]
        
        # 修复：体积效果在 4.4.3 中已移除
        # 设置体积效果
        if hasattr(eevee_settings, 'use_volumetrics'):
            eevee_settings.use_volumetrics = config["eevee_volumetric"]
        elif hasattr(eevee_settings, 'volumetric'):
            eevee_settings.volumetric.enabled = config["eevee_volumetric"]
